...
data Tm  (Ξ : ℕ) {depth : Size} : Set where
  ...
  tm  : {i : Size< depth} (σ : Op m) → (xs : Vec (Tm Ξ {i}) m) → Tm Ξ

-- `Term` stands for closed terms 
Term : Set
Term = Tm 0

Tms : VCxt → {depth : Size} → ℕ → Set
Tms Ξ {depth} n = Vec (Tm Ξ {depth}) n
...
inject₁-tm : {depth : Size} → Tm Ξ {depth} → Tm (suc Ξ) {depth}
inject₁-tm (var x)  = var  (F.inject₁ x)
inject₁-tm (tm k x) = tm k (map inject₁-tm x)
