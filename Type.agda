...
data RawArg (n : ℕ)(xs : Names) : Set where
  none  :                                       RawArg n xs 
  bvar  : (i : Fin n)                         → RawArg n xs
  fvar  : (x : Name) (x∈xs : x ∈ xs)          → RawArg n xs
...
data RawTy (n : ℕ)(xs : Names) : Set where
  At  : (P : Name) → (as : RawArgs n xs)
        → RawTy n xs
  _⇒̇_ : (F₁ F₂ : RawTy n xs)
        → RawTy n xs
  ∀̇_  : RawTy (suc n) xs
...
-- --
-- module T {xs : Names} where
--   private
--     instArg : ∀ {n} x → RawArg (suc n) xs → RawArg n (insert x xs)
--     instArg {n} x (bvar i) with n ≟ℕ (toℕ i)
--     ... | yes p = fvar x (∈-insert-= xs)
--     ... | no ¬p = bvar (lower₁ i ¬p)
--     instArg x (fvar y y∈xs) = fvar y (∈-insert xs y∈xs)
--     instArg x none          = none
--     instArg x (κ K a b)     = κ K (instArg x a) (instArg x b)
  
--   inst : ∀ {n} x → RawTy (suc n) xs → RawTy n (insert x xs)
...
--   --   absArg {n} x (fvar_ y ⦃ y∈x∷xs ⦄ ) with x ≟ y
--   --   ... | yes p = bvar (fromℕ n)
--   --   ... | no ¬p = fvar y ? -- ⦃ ∈-∷-≢ (λ {refl → ¬p refl}) y∈x∷xs ⦄
--   --   absArg x none      = none
